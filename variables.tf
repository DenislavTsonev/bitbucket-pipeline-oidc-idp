variable "name" {
  type        = string
  description = "The name of the resources"
  default     = ""
}
variable "role_per_repository" {
  type        = bool
  description = "(optional) If enabled it will create IAM role for each repository UUID"
  default     = false
}
variable "url" {
  type        = string
  description = "(Required) The URL of the identity provider. Corresponds to the iss claim."
}

variable "client_id_list" {
  type        = list(string)
  description = "(Required) A list of client IDs (also known as audiences). When a mobile or web app registers with an OpenID Connect provider, they establish a value that identifies the application. (This is the value that's sent as the client_id parameter on OAuth requests.)"
}


variable "thumbprint_list" {
  type        = list(string)
  description = "(Required) A list of server certificate thumbprints for the OpenID Connect (OIDC) identity provider's server certificate(s)."
}

variable "repository_uuids" {
  type        = list(string)
  description = "Repository UUID necessary for IAM role"
}

variable "bitbucket_public_ips" {
  type        = list(string)
  description = "(optional) Allowing only requests from Bitbucket Pipelines IP range to assume the role"
  default = [
    "34.199.54.113/32",
    "34.232.25.90/32",
    "34.232.119.183/32",
    "34.236.25.177/32",
    "35.171.175.212/32",
    "52.54.90.98/32",
    "52.202.195.162/32",
    "52.203.14.55/32",
    "52.204.96.37/32",
    "34.218.156.209/32",
    "34.218.168.212/32",
    "52.41.219.63/32",
    "35.155.178.254/32",
    "35.160.177.10/32",
    "34.216.18.129/32",
    "3.216.235.48/32",
    "34.231.96.243/32",
    "44.199.3.254/32",
    "174.129.205.191/32",
    "44.199.127.226/32",
    "44.199.45.64/32",
    "3.221.151.112/32",
    "52.205.184.192/32",
    "52.72.137.240/32",
    "104.192.136.0/21",
    "185.166.140.0/22",
    "18.205.93.0/25",
    "18.234.32.128/25",
    "13.52.5.0/25"
  ]
}

variable "policy_arn_per_repo" {
  type        = map(string)
  description = "(optional) If we have IAM role per repo enabled, we need to provide policies (ARN) for each role that module will create. Format should be repo UUID = policy arn"
  default     = {}
}
